...
class AdminModule{
   ...
   `<h3 class="w-100 d-flex justify-content-center mt-5">Администрирование</h3>
    <div class="w-100 p-3 d-flex justify-content-center">
        <form action="changeRole" method="POST">
            <div class="card border-0 m-2" style="width: 51rem;">
                ...
                <div class="mb-3 row">
                    <label for="amountSpent" class="col-sm-4 col-form-label text-end">Статистика магазина: </label>
                    <div class="col-sm-8">
                        <span id="amountSpent">109.00€</span>
                    </div>
                </div>
            </div>
        </form>
    ...
    .then(response => {
        ...
        amountSpent.textContent = response.amountSpent; // Assuming the server response includes the "amountSpent" field
    })
    .catch(error => document.getElementById('info').innerHTML = 'error: ' + error);
